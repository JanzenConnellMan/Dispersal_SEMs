...
Distance.Func <- function(FC,Alpha,M,M2,Yb,Xb,Y,P){
  ...
  # CNDD
  Tab.P    <- table(factor(M2,levels=1:S))
  N.P <- Tab.P[names(Tab.P)==FC]  
  PredCons                 <- (b*N.P)^c
  
  # Seeds 
  Seeds    <- Y*DISP.Seed*exp(-Alpha*PredCons)
...
for(mm in 1:TimeSteps){
  ...
  Replacements <- sapply(1:Replaceb, function(x){  # function that determines which speices replaces disturbed patches (apply function loops over all distrubed)
    ...
    Rad2 <- 7
    M2   <- mat.torus(Mat.S2,Rad2,x.val[x],y.val[x])
...
write.csv(df.PropsM,"TS_Super_E_48_D_7p5_c_2p5.csv",quote=F,row.names=F)
write.csv(Mat.S,"DIST_Super_E_48_D_7p5_c_2p5.csv",quote=F,row.names=F)
